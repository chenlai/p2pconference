 ...
 * Filename         : DSPGAudioSource.cpp
 * Description      : Implemented based on android AudioSource.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGAudioSource"
#include <utils/Log.h>
...
DSPGAudioSource::DSPGAudioSource(
...
status_t DSPGAudioSource::initCheck() const {
    return mInitCheck;
}

status_t DSPGAudioSource::start(MetaData *params) {
    if (mStarted) {
...
sp<MetaData> DSPGAudioSource::getFormat() {
    if (mInitCheck != OK) {
        return 0;
    }
...
void DSPGAudioSource::rampVolume(
...
status_t DSPGAudioSource::read(
    ...
    *out = NULL;
    ...
    while (mStarted) {
        ...
        mFrameCounter++;
        LOGD("AEncode: OMXRead->ASource Read, Num: %d, time=%lld", mFrameCounter, getNowms());
#endif
...
#ifdef ENABLE_PROFILING
        LOGD("AEncode: OMXRead->ASource Read DONE, Num: %d, time=%lld", mFrameCounter, getNowms());
#endif
...
void DSPGAudioSource::trackMaxAmplitude(int16_t *data, int nSamples) {
...
int16_t DSPGAudioSource::getMaxAmplitude() {

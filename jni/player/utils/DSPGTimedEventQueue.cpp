...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGTimedEventQueue"
#include <utils/Log.h>
...
DSPGTimedEventQueue::DSPGTimedEventQueue()
    : mNextEventID(1),
      mRunning(false),
      mStopped(false) {
}
...
void DSPGTimedEventQueue::start() {
    if (mRunning) {
        return;
    }
...
void DSPGTimedEventQueue::stop(bool flush) {
    if (!mRunning) {
        return;
    }
...
DSPGTimedEventQueue::event_id DSPGTimedEventQueue::postEvent(const sp<Event> &event) {
...
DSPGTimedEventQueue::event_id DSPGTimedEventQueue::postEventToBack(
        const sp<Event> &event) {
    return postTimedEvent(event, INT64_MAX);
}

DSPGTimedEventQueue::event_id DSPGTimedEventQueue::postEventWithDelay(
        const sp<Event> &event, int64_t delay_us) {
    CHECK(delay_us >= 0);
    return postTimedEvent(event, getRealTimeUs() + delay_us);
}

DSPGTimedEventQueue::event_id DSPGTimedEventQueue::postTimedEvent(
...
static bool MatchesEventID(
        void *cookie, const sp<DSPGTimedEventQueue::Event> &event) {
    DSPGTimedEventQueue::event_id *id =
        static_cast<DSPGTimedEventQueue::event_id *>(cookie);
...
bool DSPGTimedEventQueue::cancelEvent(event_id id) {
    if (id == 0) {
        return false;
    }
...
void DSPGTimedEventQueue::cancelEvents(
...
// static
int64_t DSPGTimedEventQueue::getRealTimeUs() {
    struct timeval tv;
...
// static
void *DSPGTimedEventQueue::ThreadWrapper(void *me) {
    ...
    LOGD("DSPG TimeEventQueue : prior=%d", getpriority(PRIO_PROCESS, 0) );

    LOGD("Priority set is ANDROID_PRIORITY_URGENT_AUDIO !!!!!!!!!!!");
    //setpriority(PRIO_PROCESS, 0, ANDROID_PRIORITY_URGENT_AUDIO);
    //set_sched_policy(androidGetTid(), SP_BACKGROUND);

    LOGD("DSPG TimeEventQueue 2 : prior=%d", getpriority(PRIO_PROCESS, 0) );

    static_cast<DSPGTimedEventQueue *>(me)->threadEntry();
...
void DSPGTimedEventQueue::threadEntry() {
    prctl(PR_SET_NAME, (unsigned long)"DSPGTimedEventQueue", 0, 0, 0);
...
sp<DSPGTimedEventQueue::Event> DSPGTimedEventQueue::removeEventFromQueue_l(
        event_id id) {

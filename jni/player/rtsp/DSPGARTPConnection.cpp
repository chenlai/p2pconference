 ...
 * Filename         : DSPGARTPConnection.cpp
 * Description      : Implemented based on android ARTPConnection.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGARTPConnection"
#include <utils/Log.h>

#include "DSPGARTPConnection.h"

#include "DSPGARTPSource.h"
...
// static
const int64_t DSPGARTPConnection::kSelectTimeoutUs = 2000ll;

struct DSPGARTPConnection::StreamInfo {
    int mRTPSocket;
    int mRTCPSocket;
    sp<DSPGASessionDescription> mSessionDesc;
    size_t mIndex;
    sp<AMessage> mNotifyMsg;
    KeyedVector<uint32_t, sp<DSPGARTPSource> > mSources;
...
DSPGARTPConnection::DSPGARTPConnection(uint32_t flags)
    : mFlags(flags),
      mPollEventPending(false),
      mLastReceiverReportTimeUs(-1) {
}

DSPGARTPConnection::~DSPGARTPConnection() {
}

void DSPGARTPConnection::addStream(
        int rtpSocket, int rtcpSocket,
        const sp<DSPGASessionDescription> &sessionDesc,
        size_t index,
...
void DSPGARTPConnection::removeStream(int rtpSocket, int rtcpSocket) {
...
// static
void DSPGARTPConnection::MakePortPair(
...
void DSPGARTPConnection::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatAddStream:
        {
            onAddStream(msg);
...
void DSPGARTPConnection::onAddStream(const sp<AMessage> &msg) {
    ...
    info->mSessionDesc = static_cast<DSPGASessionDescription *>(obj.get());
...
void DSPGARTPConnection::onRemoveStream(const sp<AMessage> &msg) {
...
status_t DSPGARTPConnection::start() {
    LOGV("DSPGARTPConnection::start");
    mFlags |= kStarted;
    mLastrtpTime = 0xffff;
    mFrameCounter = 0;
    return true;
...
status_t DSPGARTPConnection::decrementFrameCounter() {
    LOGD("DSPGARTPConnection::decrementFrameCounter");
    mFrameCounter--;
    return true;
}
#endif
...
void DSPGARTPConnection::postPollEvent() {
...
void DSPGARTPConnection::onPollStreams() {
    mPollEventPending = false;
    ...
    if (mLastReceiverReportTimeUs <= 0
        ...
        for (List<StreamInfo>::iterator it = mStreams.begin();
            ...
            for (size_t i = 0; i < s->mSources.size(); ++i) {
                sp<DSPGARTPSource> source = s->mSources.valueAt(i);
...
status_t DSPGARTPConnection::receive(StreamInfo *s, bool receiveRTP) {
    LOGV("receiving %s", receiveRTP ? "RTP" : "RTCP");

    CHECK(!s->mIsInjected);

//DSPG Code Change Start
    //make buffer of 10k only which is more than enough
    sp<ABuffer> buffer = new ABuffer(10240);
//DSPG Code Change End
...
status_t DSPGARTPConnection::parseRTP(StreamInfo *s, const sp<ABuffer> &buffer) {
    ...
    sp<DSPGARTPSource> source = findSource(s, srcId);

    uint32_t rtpTime = u32at(&data[4]);

#ifdef ENABLE_PROFILING
    if(rtpTime != mLastrtpTime) {
        mFrameCounter++;
        LOGD("Decode: Socket->RTPQueue, Num: %d, time=%lld", mFrameCounter, getNowms());
    }
#endif
    ...
    if ((mFlags & kFakeTimestamps) && !source->timeEstablished()) {
        ...
        LOGV("RTP Source, TimeScale=%d", source->getTimeScale() );
        source->timeUpdate(rtpTime + source->getTimeScale(), 0x100000000ll);
...
status_t DSPGARTPConnection::parseRTCP(StreamInfo *s, const sp<ABuffer> &buffer) {
    if (s->mNumRTCPPacketsReceived++ == 0) {
        sp<AMessage> notify = s->mNotifyMsg->dup();
...
status_t DSPGARTPConnection::parseBYE(
    ...
    sp<DSPGARTPSource> source = findSource(s, id);
...
status_t DSPGARTPConnection::parseSR(
        StreamInfo *s, const uint8_t *data, size_t size) {
...
sp<DSPGARTPSource> DSPGARTPConnection::findSource(StreamInfo *info, uint32_t srcId) {
    sp<DSPGARTPSource> source;
    ssize_t index = info->mSources.indexOfKey(srcId);
    if (index < 0) {
        index = info->mSources.size();

        source = new DSPGARTPSource(
...
void DSPGARTPConnection::injectPacket(int index, const sp<ABuffer> &buffer) {
...
void DSPGARTPConnection::onInjectPacket(const sp<AMessage> &msg) {
    int32_t index;
...
void DSPGARTPConnection::fakeTimestamps() {
    (new AMessage(kWhatFakeTimestamps, id()))->post();
}

void DSPGARTPConnection::onFakeTimestamps() {
    List<StreamInfo>::iterator it = mStreams.begin();
    while (it != mStreams.end()) {
        StreamInfo &info = *it++;

        for (size_t j = 0; j < info.mSources.size(); ++j) {
            sp<DSPGARTPSource> source = info.mSources.valueAt(j);

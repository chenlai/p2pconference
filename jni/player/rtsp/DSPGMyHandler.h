 ...
 * Filename         : DSPGMyHandler.h
 * Description      : Implemented based on android MyHandler.h
 *
 */
...
#ifndef DSPG_MY_HANDLER_H_

#define DSPG_MY_HANDLER_H_

//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGMyHandler"
#include <utils/Log.h>
...
struct DSPGMyHandler : public AHandler {
    DSPGMyHandler(const char *url, const sp<ALooper> &looper)
        : mLooper(looper),
          mNetLooper(new ALooper),
          mConn(new DSPGARTSPConnection),
        ...
        unsigned port;
        CHECK(DSPGARTSPConnection::ParseURL(
        ...
        switch (msg->what()) {
            ...
            case 'desc':
            {
                ...
                if (result == OK) {
                    ...
                    sp<DSPGARTSPResponse> response =
                        static_cast<DSPGARTSPResponse *>(obj.get());
                    ...
                    if (response->mStatusCode != 200) {
                        ...
                        mSessionDesc = new DSPGASessionDescription;
    ...
    void parsePlayResponse(const sp<DSPGARTSPResponse> &response) {
        mSeekable = false;
        ...
        float npt1, npt2;
        if (!DSPGASessionDescription::parseNTPRange(val.c_str(), &npt1, &npt2)) {
    ...
    sp<DSPGAPacketSource> getPacketSource(size_t index) {
        CHECK_GE(index, 0u);
...
private:
    ...
    void setupTrack(size_t index) {
        sp<DSPGAPacketSource> source =
            new DSPGAPacketSource(mSessionDesc, index);
        ...
        if (mTryTCPInterleaving) {
            ...
            unsigned rtpPort;
            DSPGARTPConnection::MakePortPair(
    ...
    DISALLOW_EVIL_CONSTRUCTORS(DSPGMyHandler);
};

}  // namespace android

#endif  // DSPG_MY_HANDLER_H_

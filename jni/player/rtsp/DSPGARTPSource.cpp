 ...
 * Filename         : DSPGARTPSource.cpp
 * Description      : Implemented based on android ARTPSource.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGARTPSource"
#include <utils/Log.h>

#include "DSPGARTPSource.h"

#include "DSPGAAMRAssembler.h"
#include "DSPGAAVCAssembler.h"
#include "DSPGAH263Assembler.h"
#include "DSPGAMPEG4AudioAssembler.h"
#include "DSPGAMPEG4ElementaryAssembler.h"
...
// DSPG Code Change End
    ...
    if (!strncmp(desc.c_str(), "H264/", 5)) {
        mAssembler = new DSPGAAVCAssembler(notify);
        mIssueFIRRequests = true;
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
//DSPG Code Change End
    } else if (!strncmp(desc.c_str(), "MP4A-LATM/", 10)) {
        mAssembler = new DSPGAMPEG4AudioAssembler(notify, params);
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
//DSPG Code Change End
        ...
        mAssembler = new DSPGAH263Assembler(notify);
        mIssueFIRRequests = true;
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
//DSPG Code Change End
        ...
        mAssembler = new DSPGAAMRAssembler(notify, false /* isWide */, params);
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
//DSPG Code Change End
    } else  if (!strncmp(desc.c_str(), "AMR-WB/", 7)) {
        mAssembler = new DSPGAAMRAssembler(notify, true /* isWide */, params);
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
//DSPG Code Change End
        ...
        mAssembler = new DSPGAMPEG4ElementaryAssembler(notify, desc, params);
        mIssueFIRRequests = true;
//DSPG Code Change Start
        int32_t numChannels;
        DSPGASessionDescription::ParseFormatDesc(desc.c_str(), &mTimeScale, &numChannels);
...
void DSPGARTPSource::processRTPPacket(const sp<ABuffer> &buffer) {
...
void DSPGARTPSource::timeUpdate(uint32_t rtpTime, uint64_t ntpTime) {
    LOGV("timeUpdate");
...
bool DSPGARTPSource::queuePacket(const sp<ABuffer> &buffer) {
    ...
    if (mNumTimes == 2) {
        ...
        {
            int64_t timeUs = (int64_t) (RTP2NTP(rtpTime) * 1E6 / (111<<32));
        }
        meta->setInt64("ntp-time", RTP2NTP(rtpTime));
    }
...
uint64_t DSPGARTPSource::RTP2NTP(uint32_t rtpTime) const {
...
void DSPGARTPSource::byeReceived() {
    mAssembler->onByeReceived();
}

void DSPGARTPSource::addFIR(const sp<ABuffer> &buffer) {
...
void DSPGARTPSource::addReceiverReport(const sp<ABuffer> &buffer) {

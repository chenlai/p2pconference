 ...
 * Filename         : DSPGARTSPController.cpp
 * Description      : Implemented based on android ARTSPController.cpp
 *
 */
...
#include "DSPGMyHandler.h"
#include "DSPGARTSPController.h"
...
DSPGARTSPController::DSPGARTSPController(const sp<ALooper> &looper)
    : mState(DISCONNECTED),
      mLooper(looper),
      mSeekDoneCb(NULL),
      mSeekDoneCookie(NULL),
      mLastSeekCompletedTimeUs(-1) {
    mReflector = new AHandlerReflector<DSPGARTSPController>(this);
    looper->registerHandler(mReflector);
}

DSPGARTSPController::~DSPGARTSPController() {
    CHECK_EQ((int)mState, (int)DISCONNECTED);
    mLooper->unregisterHandler(mReflector->id());
}

status_t DSPGARTSPController::connect(const char *url) {
    ...
    mHandler = new DSPGMyHandler(url, mLooper);
...
void DSPGARTSPController::disconnect() {
    Mutex::Autolock autoLock(mLock);
...
void DSPGARTSPController::seekAsync(
        int64_t timeUs,
...
size_t DSPGARTSPController::countTracks() {
    if (mHandler == NULL) {
        return 0;
    }
...
sp<MediaSource> DSPGARTSPController::getTrack(size_t index) {
    CHECK(mHandler != NULL);

    return mHandler->getPacketSource(index);
}

sp<MetaData> DSPGARTSPController::getTrackMetaData(
...
void DSPGARTSPController::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
...
int64_t DSPGARTSPController::getNormalPlayTimeUs() {
    CHECK(mHandler != NULL);
    return mHandler->getNormalPlayTimeUs();
}

int64_t DSPGARTSPController::getQueueDurationUs(bool *eos) {
    *eos = true;

    int64_t minQueuedDurationUs = 0;
    for (size_t i = 0; i < mHandler->countTracks(); ++i) {
        sp<DSPGAPacketSource> source = mHandler->getPacketSource(i);

 ...
 * Filename         : DSPGAAVCAssembler.cpp
 * Description      : Implemented based on android AAVCAssembler.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGAAVCAssembler"
#include <utils/Log.h>
...
// static
DSPGAAVCAssembler::DSPGAAVCAssembler(const sp<AMessage> &notify)
    : mNotifyMsg(notify),
...
DSPGAAVCAssembler::~DSPGAAVCAssembler() {
}

DSPGARTPAssembler::AssemblyStatus DSPGAAVCAssembler::addNALUnit(
        const sp<DSPGARTPSource> &source) {
    ...
    if (!mNextExpectedSeqNoValid) {
        ...
        LOGD("Not the sequence number I expected");
...
void DSPGAAVCAssembler::addSingleNALUnit(const sp<ABuffer> &buffer) {
...
bool DSPGAAVCAssembler::addSingleTimeAggregationPacket(const sp<ABuffer> &buffer) {
    const uint8_t *data = buffer->data();
    size_t size = buffer->size();
...
DSPGARTPAssembler::AssemblyStatus DSPGAAVCAssembler::addFragmentedNALUnit(
...
void DSPGAAVCAssembler::submitAccessUnit() {
...
DSPGARTPAssembler::AssemblyStatus DSPGAAVCAssembler::assembleMore(
        const sp<DSPGARTPSource> &source) {
...
void DSPGAAVCAssembler::packetLost() {
...
void DSPGAAVCAssembler::onByeReceived() {
    sp<AMessage> msg = mNotifyMsg->dup();

 ...
 * Filename         : DSPGVideoSource.h
 * Description      : Implemented based on android VideoSource.h
 *
 */
...
#ifndef DSPG_VIDEO_SOURCE_H_

#define DSPG_VIDEO_SOURCE_H_
...
class DSPGVideoSource : public MediaSource {
    static const int32_t kFramerate = 24;  // fps

public:
    DSPGVideoSource(int width, int height)
        : mWidth(width),
          mHeight(height),
...
private:
    ...
    DSPGVideoSource(const VideoSource &);
    DSPGVideoSource &operator=(const DSPGVideoSource &);
};

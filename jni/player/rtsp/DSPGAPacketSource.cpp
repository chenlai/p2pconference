 ...
 * Filename         : DSPGAPacketSource.cpp
 * Description      : Implemented based on android APacketSource.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGAPacketSource"
#include <utils/Log.h>
...
DSPGAPacketSource::DSPGAPacketSource(
...
// DSPG Code Change Start
      // Disable scanning IDR frames to avoid skipping of frames
      mScanForIDR(false),
      mFrameInCounter(0),
      mFrameOutCounter(0),
// DSPG Code Change End
    ...
    if (!strncmp(desc.c_str(), "H264/", 5)) {
        ...
        DSPGASessionDescription::ParseFormatDesc(
...
status_t DSPGAPacketSource::initCheck() const {
    return mInitCheck;
}

status_t DSPGAPacketSource::start(MetaData *params) {
    return OK;
}

status_t DSPGAPacketSource::stop() {
    return OK;
}

sp<MetaData> DSPGAPacketSource::getFormat() {
    return mFormat;
}
...
status_t DSPGAPacketSource::read(
...
#ifdef ENABLE_PROFILING
        mFrameOutCounter++;
        LOGD("Decode: Decoder->Packet Read, Num: %d, time=%lld", mFrameOutCounter, getNowms());
#endif
...
#ifdef ENABLE_PROFILING
        LOGD("Buffers size : %d", mBuffers.size());
#endif
...
void DSPGAPacketSource::updateNormalPlayTime_l(const sp<ABuffer> &buffer) {
    uint32_t rtpTime;
...
void DSPGAPacketSource::queueAccessUnit(const sp<ABuffer> &buffer) {
    int32_t damaged;
...
#ifdef ENABLE_PROFILING
    mFrameInCounter++;
    LOGD("Decode: APacketSource->Queue accu, Num: %d, time=%lld", mFrameInCounter, getNowms());
#endif
...
void DSPGAPacketSource::signalEOS(status_t result) {
    CHECK(result != OK);
...
void DSPGAPacketSource::flushQueue() {
    Mutex::Autolock autoLock(mLock);
    mBuffers.clear();
...
int64_t DSPGAPacketSource::getNormalPlayTimeUs() {
    Mutex::Autolock autoLock(mLock);
    return mLastNormalPlayTimeUs;
}

void DSPGAPacketSource::setNormalPlayTimeMapping(
...
int64_t DSPGAPacketSource::getQueueDurationUs(bool *eos) {
    Mutex::Autolock autoLock(mLock);

 ...
 * Filename         : DSPGARTSPConnection.cpp
 * Description      : Implemented based on android ARTSPConnection.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGARTSPConnection"
#include <utils/Log.h>

#include "DSPGARTSPConnection.h"
...
// static
const int64_t DSPGARTSPConnection::kSelectTimeoutUs = 1000ll;

DSPGARTSPConnection::DSPGARTSPConnection()
    : mState(DISCONNECTED),
      mAuthType(NONE),
      mSocket(-1),
      mConnectionID(0),
...
void DSPGARTSPConnection::connect(const char *url, const sp<AMessage> &reply) {
...
void DSPGARTSPConnection::disconnect(const sp<AMessage> &reply) {
    sp<AMessage> msg = new AMessage(kWhatDisconnect, id());
    msg->setMessage("reply", reply);
    msg->post();
}

void DSPGARTSPConnection::sendRequest(
...
void DSPGARTSPConnection::observeBinaryData(const sp<AMessage> &reply) {
    sp<AMessage> msg = new AMessage(kWhatObserveBinaryData, id());
    msg->setMessage("reply", reply);
    msg->post();
}

void DSPGARTSPConnection::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatConnect:
            onConnect(msg);
            break;
...
// static
bool DSPGARTSPConnection::ParseURL(
...
void DSPGARTSPConnection::onConnect(const sp<AMessage> &msg) {
    ++mConnectionID;
...
void DSPGARTSPConnection::onDisconnect(const sp<AMessage> &msg) {
...
void DSPGARTSPConnection::onCompleteConnection(const sp<AMessage> &msg) {
    sp<AMessage> reply;
    CHECK(msg->findMessage("reply", &reply));
...
void DSPGARTSPConnection::onSendRequest(const sp<AMessage> &msg) {
    sp<AMessage> reply;
    CHECK(msg->findMessage("reply", &reply));
...
void DSPGARTSPConnection::onReceiveResponse() {
    mReceiveResponseEventPending = false;
...
void DSPGARTSPConnection::flushPendingRequests() {
...
void DSPGARTSPConnection::postReceiveReponseEvent() {
...
status_t DSPGARTSPConnection::receive(void *data, size_t size) {
    size_t offset = 0;
    while (offset < size) {
...
bool DSPGARTSPConnection::receiveLine(AString *line) {
    line->clear();
...
sp<ABuffer> DSPGARTSPConnection::receiveBinaryData() {
    uint8_t x[3];
    if (receive(x, 3) != OK) {
        return NULL;
    }
...
bool DSPGARTSPConnection::receiveRTSPReponse() {
    AString statusLine;
    ...
    sp<DSPGARTSPResponse> response = new DSPGARTSPResponse;
...
bool DSPGARTSPConnection::handleServerRequest(const sp<DSPGARTSPResponse> &request) {
...
// static
bool DSPGARTSPConnection::ParseSingleUnsignedLong(
...
status_t DSPGARTSPConnection::findPendingRequest(
...
bool DSPGARTSPConnection::notifyResponseListener(
        const sp<DSPGARTSPResponse> &response) {
    ssize_t i;
...
bool DSPGARTSPConnection::parseAuthMethod(const sp<DSPGARTSPResponse> &response) {
...
void DSPGARTSPConnection::addAuthentication(AString *request) {
    if (mAuthType == NONE) {
        return;
    }

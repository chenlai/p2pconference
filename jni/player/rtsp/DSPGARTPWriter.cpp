 ...
 * Filename         : DSPGARTPWriter.cpp
 * Description      : Implemented based on android ARTPWriter.cpp
 *
 */
...
//#define LOG_NDEBUG 0
#define LOG_TAG "DSPGARTPWriter"
#include <utils/Log.h>

#include "DSPGARTPWriter.h"
...
DSPGARTPWriter::DSPGARTPWriter(int fd)
    : mFlags(0),
      mFd(fd),
      mLooper(new ALooper),
      mReflector(new AHandlerReflector<DSPGARTPWriter>(this)) {
    CHECK_GE(fd, 0);
...
status_t DSPGARTPWriter::addSource(const sp<MediaSource> &source) {
    mSource = source;
    return OK;
}

bool DSPGARTPWriter::reachedEOS() {
    Mutex::Autolock autoLock(mLock);
    return (mFlags & kFlagEOS) != 0;
}

status_t DSPGARTPWriter::start(MetaData *params) {
    Mutex::Autolock autoLock(mLock);
...
void DSPGARTPWriter::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStart:
        {
...
void DSPGARTPWriter::onRead(const sp<AMessage> &msg) {
...
void DSPGARTPWriter::onSendSR(const sp<AMessage> &msg) {
...
void DSPGARTPWriter::send(const sp<ABuffer> &buffer, bool isRTCP) {
    ssize_t n = sendto(
...
void DSPGARTPWriter::addSR(const sp<ABuffer> &buffer) {
...
void DSPGARTPWriter::addSDES(const sp<ABuffer> &buffer) {
    uint8_t *data = buffer->data() + buffer->size();
    data[0] = 0x80 | 1;
    data[1] = 202;  // SDES
    data[4] = mSourceID >> 24;
...
// static
uint64_t DSPGARTPWriter::GetNowNTP() {
...
void DSPGARTPWriter::dumpSessionDesc() {
    AString sdp;
    sdp = "v=0\r\n";
...
void DSPGARTPWriter::makeH264SPropParamSets(MediaBuffer *buffer) {
...
void DSPGARTPWriter::sendAVCData(MediaBuffer *mediaBuf) {
...
void DSPGARTPWriter::sendH263Data(MediaBuffer *mediaBuf) {
    CHECK_GE(kMaxPacketSize, 12u + 2u);
...
void DSPGARTPWriter::sendAMRData(MediaBuffer *mediaBuf) {
    const uint8_t *mediaData =
        (const uint8_t *)mediaBuf->data() + mediaBuf->range_offset();
